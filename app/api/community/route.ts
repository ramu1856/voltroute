...
export async function GET(request:Request){try{
 const user=await requireUser(),station=stationIdSchema.parse(new URL(request.url).searchParams.get('stationId')),db=database(),now=Date.now();
...
export async function POST(request:Request){try{
 sameOrigin(request);const user=await requireUser();await limit(`community:${user.userId}`,5000);
...
export async function DELETE(request:Request){try{
 sameOrigin(request);const user=await requireUser(),query=new URL(request.url).searchParams;
...
export async function PATCH(request:Request){try{
 sameOrigin(request);const user=await requireUser();await limit(`community-flag:${user.userId}`,5000);
