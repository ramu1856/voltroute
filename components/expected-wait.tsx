...
export function ExpectedWait({now,availability}:{now:number|null;availability:AvailabilityInfo|null}){
 ...
 const valid=queueSchema.safeParse(input).success,result=snapshot?queueAllowance(snapshot.input,snapshot.at,Math.max(now??0,snapshot.at)):null;
 const forecast=predictWaitForecast(availability);
