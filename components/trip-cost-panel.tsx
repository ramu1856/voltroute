...
function money(amount:number,currency:string,locale:string,rate:number){
  return formatCurrency(usdToDisplay(amount,rate), locale, currency);
}
export function TripCostPanel({result,now,displayCurrency,displayLocale,usdFxRate}:{result:SmartStopResult;now:number;displayCurrency:string;displayLocale:string;usdFxRate:number}){
  ...
  return <section className="trip-cost-panel" aria-label="Trip charging cost">
    ...
    <div className="trip-cost-total"><strong>{budget.total===null?'Total unavailable':money(budget.total,displayCurrency,displayLocale,usdFxRate)}</strong><span>{budget.total===null?'Missing prices or charging stops are not filled with guesses.':'Estimated in-trip charging energy only'}</span></div>
    <p>{budget.message}</p>
    {budget.total===null&&budget.knownSubtotal!==null&&<p className="known-subtotal">Known next-stop subtotal: <strong>{money(budget.knownSubtotal,displayCurrency,displayLocale,usdFxRate)}</strong>. This is not the full trip total.</p>}
    {budget.stops.map(stop=><details className="trip-cost-stop" key={stop.stationId}><summary>{stop.stationName} · {stop.energyCost===null?'Cost unavailable':money(stop.energyCost,displayCurrency,displayLocale,usdFxRate)}</summary>
      <dl><div><dt>Planned charge</dt><dd>{stop.arrivalBattery.toFixed(1)}% → {stop.targetBattery.toFixed(1)}%</dd></div><div><dt>Energy added</dt><dd>{stop.energyKwh===null?'Battery capacity needed':`${stop.energyKwh.toFixed(1)} kWh`}</dd></div><div><dt>Energy rate</dt><dd>{stop.price.rate===null?'Unavailable':`${formatCurrency(usdToDisplay(stop.price.rate,usdFxRate),displayLocale,displayCurrency,2,4)}/kWh`} · {stop.price.label}</dd></div><div><dt>Price source</dt><dd>{stop.price.sourceUrl?<a href={stop.price.sourceUrl} target="_blank" rel="noreferrer">{stop.price.source} ↗</a>:stop.price.source}</dd></div><div><dt>Battery at destination</dt><dd>{stop.reachesDestination?`~${stop.destinationBattery.toFixed(1)}%, if this charge succeeds`:'More charging stops needed'}</dd></div></dl>
    ...
    {displayCurrency!=='USD'&&<p className="muted-small">Costs are converted from USD estimates using the latest available FX rate.</p>}
  </section>;
}
