...
export function SmartStopPlanner({origin,destination,profile,battery,now,reportsVersion,enteredRates,accessToken,onResult,onClear,onView}:Props){
  ...
  async function calculate(overrideNoStranding?:boolean){
    ...
    try{
      const response=await fetch('/api/smart-stop',{method:'POST',headers:{'Content-Type':'application/json',...(accessToken?{Authorization:`Bearer ${accessToken}`}:{})},body:JSON.stringify(payload),signal:controller.signal});
