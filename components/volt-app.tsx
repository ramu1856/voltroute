...
useEffect(()=>{let cancelled=false;queueMicrotask(()=>{if(!cancelled){routeId.current++;setRoad(null);setSmartPlan(null);setTripSnapshot(null);setMapRouteOverride(null);setNavigationRoute(null);setPinnedMapRoute(null);setRouteOnlyMode(false);setNavigationTargetId(null);setNavigationStartedAt(null);setNavigationDistanceMiles(null);setNavigationEtaMinutes(null);setRouteError('');setRouteBusy(false);setRouteStationsMode(false);setRouteStationsError('');setRouteStationsBusy(false);routeStationsSnapshot.current=null;}});return()=>{cancelled=true;};},[origin,destination,profile,battery]);
