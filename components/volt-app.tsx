...
const routeMidpoint=useMemo(()=>{if(!road?.coordinates.length)return null;const mid=road.coordinates[Math.floor(road.coordinates.length/2)];if(!mid)return null;const [lon,lat]=mid;return {lat,lon};},[road]);
 ...
 function showSmartStop(result:SmartStopResult,view:'main'|'backup'='main'){
  ...
  setRouteStationsError('');
  setRouteStationsBusy(false);
  requestId.current++;routeId.current++;setLoading(false);setRouteBusy(false);setRouteError('');setError('');setRoad(null);setTripSnapshot(null);setMapRouteOverride(null);setSmartPlan(result);
