...
function PlacePicker({label,value,onSelect}:{label:string;value:Point;onSelect:(p:Point)=>void}){
 ...
 return <div className="place-picker"><label>{label}<div className="place-input"><input value={q} maxLength={160} onChange={e=>setQ(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();void search(true);}}}/><Button type="button" variant="ghost" size="icon" disabled={busy||q.trim().length<2} aria-label={`Search ${label}`} onClick={()=>void search()}><Search/></Button></div></label>
