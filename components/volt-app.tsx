...
import { detectUserCurrency, detectUserLocale } from '@/lib/currency-display';
...
function stationNavigationUrl(s:Station){
 const url=new URL('https://www.google.com/maps/dir/');
 url.search=new URLSearchParams({
  api:'1',
  destination:`${s.lat.toFixed(6)},${s.lon.toFixed(6)}`,
  travelmode:'driving',
  dir_action:'navigate',
 }).toString();
 return url.toString();
}
function openStationRoute(s:Station){
 const navigationUrl=stationNavigationUrl(s);
...
async function search(autoPickFirst=false){const id=++revision.current;setBusy(true);setError('');try{const r=await api<{data:Point[]}>(`/api/explore?action=search&q=${encodeURIComponent(q)}`);if(id!==revision.current)return;if(autoPickFirst&&r.data.length){const first=r.data[0];onSelect(first);setQ(first.label);setPlaces([]);setError('');return;}setPlaces(r.data);if(!r.data.length)setError('No matching location found. Try city, state or country.');}catch(e){if(id===revision.current)setError((e as Error).message);}finally{if(id===revision.current)setBusy(false);}}
 return <div className="place-picker"><label>{label}<div className="place-input"><input value={q} maxLength={160} onChange={e=>setQ(e.target.value)} onBlur={()=>{if(q.trim().length>=2&&q.trim()!==value.label)void search(true);}} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();void search(true);}}}/><Button type="button" variant="ghost" size="icon" disabled={busy||q.trim().length<2} aria-label={`Search ${label}`} onClick={()=>void search(true)}><Search/></Button></div></label>
...
export default function VoltApp({supabaseUrl,supabaseKey}:{supabaseUrl:string;supabaseKey:string}){
 ...
 const [directoryNotice,setDirectoryNotice]=useState(''),[amenityNotice,setAmenityNotice]=useState(''),[amenityFetchedAt,setAmenityFetchedAt]=useState(''),[searchedRadius,setSearchedRadius]=useState('80467');
 ...
 const [filter,setFilter]=useState(''),[fast,setFast]=useState(false),[match,setMatch]=useState(false),[freeOnly,setFreeOnly]=useState(false),[radius,setRadius]=useState('80467');
...
const [detailsReady,setDetailsReady]=useState(false);
 ...
 const [displayLocale]=useState(()=>detectUserLocale()),[displayCurrency]=useState(()=>detectUserCurrency(detectUserLocale())),[usdFxRate,setUsdFxRate]=useState(1);
...
const drivingDistanceByStation=useMemo(()=>selected&&selectedRoad?{[selected.id]:selectedRoad.miles}:{} as Record<string,number>,[selected,selectedRoad]);
...
useEffect(()=>{
 ...
 let cancelled=false;
 void fetch(`/api/fx?currency=${displayCurrency}`,{cache:'no-store'}).then(async response=>{
  if(!response.ok)throw new Error('fx unavailable');
  const data=await response.json() as {rate?:number};
  if(cancelled)return;
  setUsdFxRate(Number.isFinite(data.rate)&&Number(data.rate)>0?Number(data.rate):1);
 }).catch(()=>{if(!cancelled)setUsdFxRate(1);});
 return()=>{cancelled=true;};
},[displayCurrency]);
...
async function plan(input:TripInput={origin,destination,profile,battery}){setSmartPlan(null);routeStationsSnapshot.current=null;setRouteStationsMode(false);setRouteStationsError('');setRouteStationsBusy(false);const id=++routeId.current;setRouteBusy(true);setRouteError('');setRoad(null);try{const r=await api<{data:Omit<RoadRoute,'fetchedAt'>;fetchedAt:string}>(`/api/explore?action=route&lat=${input.origin.lat}&lon=${input.origin.lon}&toLat=${input.destination.lat}&toLon=${input.destination.lon}`);if(id!==routeId.current)return;setRoad({...r.data,fetchedAt:r.fetchedAt});setTripSnapshot(input);}catch(e){if(id===routeId.current)setRouteError((e as Error).message);}finally{if(id===routeId.current)setRouteBusy(false);}}
 ...
 <header className="topbar"><Link href="/" className="brand"><span className="brand-mark"><Zap/></span>VoltRoute <small className="beta">BETA</small></Link><div className="account-nav">{signedIn?<><span>{account||session?.user.email||session?.user.phone||'Signed in'}</span><button type="button" onClick={()=>void supabaseBrowser({url:supabaseUrl,key:supabaseKey}).auth.signOut()}>Sign out</button></>:<button type="button" disabled={!authReady} onClick={()=>void requestGoogleSignIn()}>{authReady?'Continue with Google':'Checking account…'}</button>}</div></header>
  ...
  <nav className="mobile-jumps" aria-label="Charging page sections"><a href="#charger-search">Search & vehicle</a><a href="#charger-results">Chargers</a><a href="#charging-stop">Charging stop</a></nav>
  <details className="quick-guide"><summary>New to VoltRoute? Start here</summary>
   <ol>
    ...
    <li><strong>Find a charging stop</strong><p>Search any city worldwide or use Near me. Choose a 50–250-mile search area. The directory returns up to 250 mapped results, so it may not show every charger.</p><a href="#charger-search" onClick={()=>setWorkspaceTab('explore')}>Open charger search ↓</a></li>
  ...
  <div className="vr-workspace">
   <aside id="charger-search" className="vr-controls"><p className="eyebrow">Find your next stop</p><h1>Charge up.<br/>Take a break.</h1>
    ...
    <TabsContent value="explore" className="tab-body"><PlacePicker label="Search a city or place worldwide" value={origin} onSelect={pickOrigin}/>
     <div className="inline-controls"><Select value={radius} onValueChange={setRadius}><SelectTrigger aria-label="Search radius"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="80467">50-mile area</SelectItem><SelectItem value="160934">100-mile area</SelectItem><SelectItem value="241402">150-mile area</SelectItem><SelectItem value="321869">200-mile area</SelectItem><SelectItem value="402336">250-mile area</SelectItem></SelectContent></Select><Button variant="outline" aria-label="Use current location" disabled={locating} onClick={locate}><LocateFixed/>{locating?'Locating…':'Near me'}</Button></div>
    ...
    <TabsContent value="trip" className="tab-body"><PlacePicker label="From" value={origin} onSelect={pickOrigin}/><PlacePicker label="To" value={destination} onSelect={setDestination}/>
     ...
     <div className="trip-route-actions"><Button variant="outline" className="full" disabled={routeStationsBusy} onClick={()=>void showRouteStationDots()}><MapPin/>{routeStationsBusy?'Loading route charger list…':'Show route chargers in list'}</Button>{routeStationsMode&&<Button variant="outline" className="full" onClick={restoreLocalStationView}><RotateCcw/>Back to previous station view</Button>}</div>
     {routeStationsMode&&<p className="muted-small">Route charger mode is active. The charger list now focuses on stations near the full road corridor.</p>}
     ...
     <Button variant="secondary" className="full" disabled={saving} onClick={()=>save('trip',tripSnapshot)}><Bookmark/>Save trip</Button><Button asChild variant="outline" className="full"><a href={tripDirections({lat:tripSnapshot.origin.lat,lon:tripSnapshot.origin.lon},{lat:tripSnapshot.destination.lat,lon:tripSnapshot.destination.lon})} target="_blank" rel="noreferrer"><Navigation/>Open full route in Google Maps</a></Button><Button asChild variant="outline" className="full"><a href={googleEvSearch(routeMidpoint||{lat:Number(((tripSnapshot.origin.lat+tripSnapshot.destination.lat)/2).toFixed(4)),lon:Number(((tripSnapshot.origin.lon+tripSnapshot.destination.lon)/2).toFixed(4))})} target="_blank" rel="noreferrer"><Search/>Find EV chargers on this drive</a></Button></section>}
    </TabsContent>
  ...
  <section id="charger-results" className="vr-main" aria-label="Charger results" tabIndex={0}><div className="vr-map-tools"><label className="station-search"><Search size={18}/><input aria-label="Filter loaded stations" placeholder="Filter station name or network" value={filter} onChange={e=>setFilter(e.target.value)}/></label><label className="vr-toggle"><Switch checked={fast} onCheckedChange={setFast}/>100+ kW</label><label className="vr-toggle"><Switch checked={match} onCheckedChange={setMatch}/>My connector</label><label className="vr-toggle"><Switch checked={freeOnly} onCheckedChange={setFreeOnly}/>Listed as free</label><label className="vr-toggle"><Switch checked={openNow} onCheckedChange={setOpenNow}/>Open now</label><label className="vr-toggle"><Switch checked={allDay} onCheckedChange={setAllDay}/>24/7</label><p className="hours-filter-note">Opening-hours filters use listed schedules in station local time. They do not indicate free charging ports.</p></div>
    ...
    {smartPlan&&<TripCostPanel result={smartPlan} now={currentTime} displayCurrency={displayCurrency} displayLocale={displayLocale} usdFxRate={usdFxRate}/>} 
    ...
    <section className="results-panel" aria-busy={loading}><div className="results-heading"><div><p className="eyebrow">{emergency?'Emergency compatible search':center.label}</p><h2>{loading?'Finding mapped chargers…':error?'Charger search unavailable':fetchedAt?`${visible.length} mapped charging stations`:'Search for charging stations'}</h2></div>{fetchedAt&&!loading&&<small>{directoryNotice?'Saved directory retrieved':'Directory retrieved'}<br/>{evidenceTime(fetchedAt)}</small>}</div>
     <p className="muted-small">Up to 250 mapped results in the selected area, sorted by distance. Coverage may be incomplete. Distances update to driving miles after you select a charger. Food/restrooms load when you select a charger.</p>
