...
function drivingDirections(origin:Point,destination:Point,navigate=false){
  ...
  <div className="vr-workspace">
   <aside id="charger-search" className="vr-controls"><p className="eyebrow">Find your next stop</p><h1>Charge up.<br/>Take a break.</h1>
    ...
    <TabsContent value="trip" className="tab-body"><PlacePicker label="From" value={origin} onSelect={pickOrigin}/><PlacePicker label="To" value={destination} onSelect={setDestination}/>
     ...
     <Button variant="secondary" className="full" disabled={saving} onClick={()=>save('trip',tripSnapshot)}><Bookmark/>Save trip</Button><Button asChild className="full"><a href={drivingDirections(tripSnapshot.origin,tripSnapshot.destination,true)} target="_blank" rel="noreferrer"><Navigation/>Start in Google Maps</a></Button><Button asChild variant="outline" className="full"><a href={googleEvSearch(routeMidpoint||{lat:Number(((tripSnapshot.origin.lat+tripSnapshot.destination.lat)/2).toFixed(4)),lon:Number(((tripSnapshot.origin.lon+tripSnapshot.destination.lon)/2).toFixed(4))})} target="_blank" rel="noreferrer"><Search/>Find EV chargers on this drive</a></Button><a className="text-link" href={drivingDirections(tripSnapshot.origin,tripSnapshot.destination)} target="_blank" rel="noreferrer">Open route preview ↗</a></section>}
    </TabsContent>
